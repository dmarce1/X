...
template<class T>
EXPORT_GLOBAL
void hydro_compute_u(state_var<T>& u00, state_var<T>& dU, int N[NDIM], T dt) {
...
__global__
void hydro_gpu_compute_u(real* U, state_var<real>* dU, int nx, int ny, int nz, real dt) {
	int N[NDIM] = { nx, ny, nz };
	...
	state_var<real> Uidx(U, idx, D);
	hydro_compute_u(Uidx, dU[idx], N, dt);
	set_state_var(U, idx, Uidx, D);
}

void hydro_cpu_compute_u(real* U, state_var<real>* dU, int nx, int ny, int nz, real dt) {
	int N[NDIM] = { nx, ny, nz };
	const int D = nx * ny * nz;
	for (int zi = BW; zi < nz - BW; ++zi) {
		for (int yi = BW; yi < ny - BW; ++yi) {
...
void hydro_kernel(device_vector<real>& U, device_vector<state_var<real>>& dU, device_vector<double>& a, int nx, int ny, int nz, real dx, real dy, real dz, int rk) {
#ifdef USE_CPU
...
void hydro_compute_u(device_vector<real>& U, device_vector<state_var<real>>& dU, int nx, int ny, int nz, real dt) {
#ifdef USE_CPU
	hydro_cpu_compute_u(U.data(), dU.data(), nx, ny, nz, dt);
#else
	dim3 threads(nx - 2 * BW);
	dim3 blocks(ny - 2 * BW, nz - 2 * BW);
	hydro_gpu_compute_u<<<blocks,threads>>>(U.data().get(), dU.data().get(), nx, ny, nz, dt );
#endif
}
